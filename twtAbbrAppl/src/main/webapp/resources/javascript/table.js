	...
	table.filter = function(o,filters,args) {
		...
		if (!filters) {
			...
			for (var i=0,L=filters.length; i<L; i++) {
				...
				// Apply the passed-in filters to the existing list of filters for the table, removing those that have a filter of "All"
				if (filter.filter=="All" && tdata.filters) {
			...
			if (hasClass(cell,table.FilterableClassName)) {
				...
				if (colValues.length>0) {
					if (typeof(args.insert)=="function") {
						...
						var sel = '<select onchange="Table.filter(this,this)" onclick="Table.cancelBubble(event)" class="'+table.AutoFilterClassName+'"><option value="All">'+table.FilterAllLabel+'</option>';
