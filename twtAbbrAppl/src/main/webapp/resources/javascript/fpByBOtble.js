...
table.refreshHoursSummary = function( o,filters, args ) {
 var t = [], 
  ...
  for (j = t.length; j>=14; j--) {
    if( forecastrow.cells[j-13] ) { forecastrow.cells[j-13].firstChild.nodeValue = t[j].toFixed(2); }
  }
  	args = args || {};
	...
	for (col in tdata.filters) {
		...
		if (col==6) {		document.getElementById( 'selSi' ).value=tdata.filters[col]; }
		if (col==7) {		document.getElementById( 'selPmtName' ).value=tdata.filters[col]; }
		if (col==8) {		document.getElementById( 'selPmtTitle' ).value=tdata.filters[col]; }
		if (col==10) {		document.getElementById( 'selWorkItemName' ).value=tdata.filters[col]; }
	}
