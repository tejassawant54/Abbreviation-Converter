...
//alert("col = " + col);
//alert("selected filter = " + tdata.filters[col]);
//}
...
table.refreshHoursSummary = function( o,filters, args ) {
 var t = [], 
 	 cell,
	 row = document.getElementById( 'forceplan' ).rows,
  	 i = row.length,
  	 lastrow = row[i];
	...
	if (row[i].style.display=="") 
	{
	  while(j--) {
		...
		if (cell[j].firstChild != null) {
			if (cell[j].firstChild.nodeValue == null) {
				t[j] += parseFloat( cell[j].firstChild.firstChild.nodeValue ) || 0;
			} else {
  ...
  var sumrows = document.getElementById( 'forcePlanSummary' ).rows,
  forecastrow = sumrows[2],

  cell = forecastrow.cells,
  sumColCnt = cell.length;

  for (j = t.length; j>=15; j--) {
    if( forecastrow.cells[j-14] ) { forecastrow.cells[j-14].firstChild.nodeValue = t[j].toFixed(2); }

  }
  	args = args || {};

	var t1 = this.resolve(document.getElementById( 'forceplan' ),args);
	var tdata = this.tabledata[t1.id];
	var maxcol = 0;
	// reset hidden elements to no values
	document.getElementById( 'selResId' ).value="";
	document.getElementById( 'selResName' ).value="";
	document.getElementById( 'selVpName' ).value="";
	document.getElementById( 'selBoName' ).value="";
	document.getElementById( 'selLaborClass' ).value="";
	document.getElementById( 'selCostType' ).value="";
	document.getElementById( 'selCostGroup' ).value="";
	document.getElementById( 'selPiw' ).value="";
	document.getElementById( 'selSi' ).value="";
	document.getElementById( 'selPmtName' ).value="";
	document.getElementById( 'selPmtTitle' ).value="";
	document.getElementById( 'selApplicationName' ).value="";
	document.getElementById( 'selWorkItemName' ).value="";
			
	for (col in tdata.filters) {
		if (maxcol < col) {
			maxcol = col;
		}
		if (col==0) {		document.getElementById( 'selResId' ).value=tdata.filters[col]; }
		if (col==1) {		document.getElementById( 'selResName' ).value=tdata.filters[col]; }
		if (col==2) {		document.getElementById( 'selVpName' ).value=tdata.filters[col]; }
		if (col==3) {		document.getElementById( 'selBoName' ).value=tdata.filters[col]; }
		if (col==4) {		document.getElementById( 'selLaborClass' ).value=tdata.filters[col]; }
		if (col==5) {		document.getElementById( 'selCostType' ).value=tdata.filters[col]; }
		if (col==6) {		document.getElementById( 'selCostGroup' ).value=tdata.filters[col]; }
		if (col==7) {		document.getElementById( 'selPiw' ).value=tdata.filters[col]; }
		if (col==8) {		document.getElementById( 'selSi' ).value=tdata.filters[col]; }
		if (col==9) {		document.getElementById( 'selPmtName' ).value=tdata.filters[col]; }
		if (col==10) {		document.getElementById( 'selPmtTitle' ).value=tdata.filters[col]; }
		if (col==11) {		document.getElementById( 'selApplicationName' ).value=tdata.filters[col]; }
		if (col==13) {		document.getElementById( 'selWorkItemName' ).value=tdata.filters[col]; }
	}
	
	if (maxcol > 4) {
		sumrows[1].style.display="none";
		sumrows[3].style.display="none";
	}
	else
	{
		sumrows[1].style.display="";
		sumrows[3].style.display="";
	}
	...
	table.autofilter = function(t,args) {
		...
		table.processTableCells(t, "THEAD", function(cell) {
			if (hasClass(cell,table.FilterableClassName)) {
				...
				var nameIds = table.getUniqueColValues(t,1);
