...
table.refreshFtpSummary = function( o,filters, args ) {

 var t = [], 
 	 cell,
	 row = document.getElementById( 'authorizations' ).rows,
  	 i = row.length,
  	 lastrow = row[i];
	  ...
	  while(j--) {
		...
		if (cell[j].firstChild != null) {
			if (cell[j].firstChild.nodeValue == null) {
				t[j] += parseFloat( cell[j].firstChild.value ) || 0;
			} else {
  ...
  var sumrows = document.getElementById( 'authorSummary' ).rows,
  forecastrow = sumrows[1],
  cell = forecastrow.cells,
  sumColCnt = cell.length;

  
  forecastrow.cells[0].firstChild.nodeValue = t[3].toFixed(2);
  forecastrow.cells[1].firstChild.nodeValue = t[4].toFixed(2);
  forecastrow.cells[2].firstChild.nodeValue = t[5].toFixed(2);
  forecastrow.cells[3].firstChild.nodeValue = (t[4] - t[3]).toFixed(2);
  forecastrow.cells[4].firstChild.nodeValue = (t[5] - t[3]).toFixed(2);  
  forecastrow.cells[5].firstChild.nodeValue = (t[5] - t[4]).toFixed(2);  
      
  	args = args || {};

	var t1 = this.resolve(document.getElementById( 'authorizations' ),args);
