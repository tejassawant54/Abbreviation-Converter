...
table.refreshHoursSummary = function( o,filters, args ) {
 var t = [], 
	  ...
	  while(j--) {
		...
		if (cell[j].firstChild != null) {
			if (cell[j].firstChild.nodeValue == null) {
				//alert("col " + j + " classname " + cell[j].firstChild.className);
				if (cell[j].firstChild.className == "sameBO") {
					t[j] += parseFloat( cell[j].firstChild.firstChild.nodeValue ) || 0;
					//alert("forecast " + t[j]);
				} else {
					oth[j] += parseFloat( cell[j].firstChild.firstChild.nodeValue ) || 0;
					//alert("other forecast " + oth[j]);
				}
			} else {
  ...
  otherforecastrow = sumrows[3],
  cell = forecastrow.cells,
  sumColCnt = cell.length;

  for (j = t.length; j>=11; j--) {
    if( forecastrow.cells[j-10] ) { forecastrow.cells[j-10].firstChild.nodeValue = t[j]; }
    if( otherforecastrow.cells[j-10] ) {otherforecastrow.cells[j-10].firstChild.nodeValue = oth[j]; }
  }
  	args = args || {};
...
//		if (col==1) {		document.getElementById( 'selResName' ).value=tdata.filters[col]; }
		if (col==2) {		document.getElementById( 'selLaborClass' ).value=tdata.filters[col]; }
		if (col==3) {		document.getElementById( 'selCostType' ).value=tdata.filters[col]; }
		if (col==4) {		document.getElementById( 'selCostGroup' ).value=tdata.filters[col]; }
		if (col==5) {		document.getElementById( 'selPiw' ).value=tdata.filters[col]; }
		if (col==6) {		document.getElementById( 'selPmtName' ).value=tdata.filters[col]; }
		if (col==7) {		document.getElementById( 'selPmtTitle' ).value=tdata.filters[col]; }
		if (col==9) {		document.getElementById( 'selWorkItemName' ).value=tdata.filters[col]; }
	}
	
	if (maxcol > 2) {
		sumrows[1].style.display="none";
		sumrows[4].style.display="none";
	}
	else
	{
		sumrows[1].style.display="";
		sumrows[4].style.display="";
	}
